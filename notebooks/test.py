...
class Detector(object):
    def __init__(self,img, boxes, scores,classes):
        self.img = img
        self.boxes = boxes
        self.scores = scores
    ...
    def print_boxes(self):
        ...
        for i in range(self.boxes.shape[0]):
            ymin = int(self.boxes[i, 0] * height)
            xmin = int(self.boxes[i, 1] * width)
            ymax = int(self.boxes[i, 2] * height)
            xmax = int(self.boxes[i, 3] * width)
            print('{:.3f} | {:.3f} | {:.3f} | {:.3f}'.format(xmin, ymin,xmax,ymax))
